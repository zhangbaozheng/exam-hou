 ...
 * @Last Modified time: yyyy-10-Th 02:18:59
 */
...
class ExamController extends Controller{
    ...
    /**
     * 学生分数
     * GET /exam_student/getscore
     */
    async score() {
        let result = await this.service.exam.exam.getscore(this.ctx.query);
        if(result.length) {
            this.ctx.body = {
                code:1,
                msg:'获取成功',
     ...
     */
    async getscores() {
        let result = await this.service.exam.exam.gradescore(this.ctx.request.body);
        if(result.length) {
            this.ctx.body = {
                code:1,
                msg:'获取成功',
