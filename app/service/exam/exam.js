 ...
 * @Last Modified time: yyyy-10-Th 02:18:21
 */
...
class ExamService extends Service {
  ...
  async beforeSubjectsQuestions(subject_id, exam_id = "") {
    ...
    let subject_index = subjects.findIndex(
      (item) => item.subject_id === subject_id
    );
    // 获取本学科之前所有的学科
    let subject_ids = subjects
      .filter((item, index) => index <= subject_index)
    ...
    if (exam_id) {
      ...
      let exam_ids = subjects
        .filter((item, index) => index <= exam_index)
        .map((item) => item.exam_index);
      // 过滤掉本次考试之后的试题：如当前考试为组件化周考二，那把周考三及月考的题过滤掉
      result = result.filter((item) => {
        if (
          item.subject_id === subject_id &&
          exam_ids.indexOf(item.exam_id) === -1
        ) {
          return false;
        }
        return true;
  ...
  async getSubjectQuestions(subject_id, exam_id, number) {
    let questions = await this.app.mysql.query(
      getRandomQuestions(subject_id, exam_id, number)
    );
    return questions;
  }
  ...
  async insertExam({ exam_id, questions, ...params }) {
    ...
    exam_content.questions = questions.map((item) => {
      let jsonContent = fs.readFileSync(
        path.join(this.app.config.questionsRootPath, item.json_path)
      );
  ...
  async allExam(querys) {
    ...
    result.forEach((item) => {
      ...
      let index = newResult.findIndex(
        (value) => value.exam_exam_id === item.exam_exam_id
      );
      if (index !== -1) {
  ...
  async getExam(exam_exam_id) {
    ...
    result = result[0];
    // 获取该id对应的试题
    result.questions = await this.app.mysql.query(
      getQuestions(result.question_ids)
    );
    // 拼接返回前端试卷内容
  ...
  //获取学生分数
  async getscore() {
    const $sql = `SELECT * FROM exam_student`;
    return await this.ctx.app.mysql.query($sql);
  }
  //获取班级学生分数
  async gradescore({grade_id}) {
    const $sql = `select * from exam_student where grade_id = ? `;
